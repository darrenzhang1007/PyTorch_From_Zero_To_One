...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")  # GPU使用
...
# ============================ step 1/5 数据 ============================
train_dir = ("H:/PyTorch_From_Zero_To_One/13_finetune/hymenoptera_data/train")
valid_dir = ("H:/PyTorch_From_Zero_To_One/13_finetune/hymenoptera_data/val")
...
flag = 1
if flag:
    path_pretrained_model = ("H:/PyTorch_From_Zero_To_One/13_finetune/15_resnet18-5c106cde.pth")
...
resnet18_ft.to(device)  # GPU使用：模型
# ============================ step 3/5 损失函数 ============================
...
if flag:
    ...
    optimizer = optim.SGD([
        {'params': base_params, 'lr': LR*0},   # 0 -> 不能够更新参数了
...
for epoch in range(start_epoch + 1, MAX_EPOCH):
    ...
    for i, data in enumerate(train_loader):
        ...
        # forward
        inputs, labels = data
        inputs, labels = inputs.to(device), labels.to(device)  # GPU使用：数据
